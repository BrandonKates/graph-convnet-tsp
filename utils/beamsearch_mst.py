...
class Beamsearch(object):
    ...
    def __init__(self, beam_size, batch_size, num_edges,
                 dtypeFloat=torch.FloatTensor, dtypeLong=torch.LongTensor, 
                 probs_type='raw', random_start=True):
        """
        Args:
            beam_size: Beam size
        ...
        self.start_edges = torch.zeros(batch_size, beam_size).type(self.dtypeLong)
        if random_start == True:
            # Random starting nodes
            self.start_edges = torch.randint(0, num_edges, (batch_size, beam_size)).type(self.dtypeLong)
        # Mask for constructing valid hypothesis
        self.mask = torch.ones(batch_size, beam_size, num_edges).type(self.dtypeFloat)
        #self.update_mask(self.num_edges)  # Mask the starting node of the beam search
        ...
        self.next_edges = [self.start_edges]
        
    def get_current_state(self):
        """Get the output of the beam at the current timestep.
        """
        current_state = (self.next_edges[-1].unsqueeze(2)
                         .expand(self.batch_size, self.beam_size, self.num_edges))
    ...
    def advance(self, trans_probs):
        ...
        # Update backpointers
        prev_k = bestScoresId / self.num_edges
        self.prev_Ks.append(prev_k)
        # Update outputs
        new_edges = bestScoresId - prev_k * self.num_edges
        self.next_edges.append(new_edges)
        # Re-index mask
        ...
        self.update_mask(new_edges)

    def update_mask(self, new_edges):
        """Sets new_nodes to zero in mask.
        """
        arr = (torch.arange(0, self.num_edges).unsqueeze(0).unsqueeze(1)
               .expand_as(self.mask).type(self.dtypeLong))
        new_edges = new_edges.unsqueeze(2).expand_as(self.mask)
        update_mask = 1 - torch.eq(arr, new_edges).type(self.dtypeFloat)
    ...
    def get_hypothesis(self, k):
        ...
        """
        #assert self.num_edges == len(self.prev_Ks) + 1
        
        hyp = -1 * torch.ones(self.batch_size, self.num_edges).type(self.dtypeLong)
        for j in range(len(self.prev_Ks) - 1, -2, -1):
            hyp[:, j + 1] = self.next_edges[j + 1].gather(1, k).view(1, self.batch_size)
