...
if notebook_mode == True and viz_mode == False:
    ...
    train_filepath = config.train_filepath
    dataset = iter(MSTReader(num_nodes, num_neighbors, batch_size, train_filepath))
    batch = next(dataset)
...
if notebook_mode==True:
    ...
    with torch.no_grad():
        ...
        # Get batch beamsearch tour prediction
        bs_nodes = beamsearch_tour_nodes_shortest(
            y_preds, x_edges_values, beam_size, batch_size, num_nodes, dtypeFloat, dtypeLong, probs_type='raw')
        #bs_nodes = beamsearch_tour_nodes(
            #y_preds, beam_size, batch_size, num_nodes, dtypeFloat, dtypeLong, probs_type='logits')
...
for f_idx, idx in enumerate(np.random.choice(len(y), 10, replace=False)):
    ...
    plot_tsp(plt2, x_coord, W_sol_bins, W_sol_probs, W_sol_bins)
    plt.show()
